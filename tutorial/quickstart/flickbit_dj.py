...
def import_and_reduce(full):
    ...
    cntr = np.genfromtxt('{0}\\quickstart\\centers.csv'.format(settings.BASE_DIR), delimiter=",")
    labels = np.genfromtxt('{0}\\quickstart\\labels.csv'.format(settings.BASE_DIR), delimiter=",")
#print(cntr)
    num_dict['director_name_num'] = read_dictionary("{0}\\quickstart\\flick_bit\\num_dict\\director.csv".format(settings.BASE_DIR))
    #with open("{0}\\static\\movie_{1}.json".format(settings.PROJECT_ROOT,cluster[0]),'r') as f:

    num_dict['actor_1_name_num'] = read_dictionary('{0}\\quickstart\\flick_bit\\num_dict\\actor_1.csv'.format(settings.BASE_DIR))
    num_dict['actor_2_name_num'] = read_dictionary('{0}\\quickstart\\flick_bit\\num_dict\\actor_2.csv'.format(settings.BASE_DIR))
    num_dict['actor_3_name_num'] = read_dictionary('{0}\\quickstart\\flick_bit\\num_dict\\actor_3.csv'.format(settings.BASE_DIR))
    num_dict['country_num'] = read_dictionary('{0}\\quickstart\\flick_bit\\num_dict\\country.csv'.format(settings.BASE_DIR))
    num_dict['genre_0_num'] = read_dictionary('{0}\\quickstart\\flick_bit\\num_dict\\genre_0.csv'.format(settings.BASE_DIR))
    num_dict['genre_1_num'] = read_dictionary('{0}\\quickstart\\flick_bit\\num_dict\\genre_1.csv'.format(settings.BASE_DIR))
...
#    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.0, shuffle=False)
    ...
    rscale = pd.read_csv('{0}\\quickstart\\flick_bit\\median.csv'.format(settings.BASE_DIR), delimiter=",")
    #print(rscale.head())
    median = rscale['0'].values.tolist()
#    median = rscale.head(0)
    #print(median)

    rscale = pd.read_csv('{0}\\quickstart\\flick_bit\\scaling.csv'.format(settings.BASE_DIR), delimiter=",")
    #print(rscale.head())
...
#    alldata = np.vstack((reduced_data[:,0]))
    ...
    kmeans = KMeans(init='random', n_clusters=10, n_init=10, max_iter=1000)
    kmeans.cluster_centers_ = cntr
    kmeans.labels_ = labels
    cluster_membership = kmeans.predict(data)
